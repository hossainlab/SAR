...
const Layout: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  return (
    <div className="min-h-screen flex flex-col bg-[#fdfdfd]">
      <header className="sticky top-0 z-50 bg-white/70 backdrop-blur-xl border-b border-slate-100">
        <nav className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-14 md:h-16 flex items-center justify-between">
          ...
          <div className="hidden md:flex items-center space-x-6 lg:space-x-8 text-sm font-semibold">
            ...
            <a href="https://docs.google.com/spreadsheets/d/19RdOnf8wdOgk1o3HMovmaNrh8Qmhmg72PCPUlxkPa68/edit?usp=sharing" target="_blank" rel="noopener noreferrer" className="text-slate-500 hover:text-indigo-600 transition-colors">Schedule</a>
          ...
          <div className="flex items-center space-x-3">
             <a href="https://forms.gle/5YmigJSJwbXsWB7eA" target="_blank" rel="noopener noreferrer" className="bg-slate-900 text-white px-3 sm:px-4 md:px-5 py-1.5 sm:py-2 rounded-full text-[11px] sm:text-[12px] md:text-sm font-bold hover:bg-slate-800 transition-all shadow-md active:scale-95">
               Enroll Now
             </a>
          </div>
        </nav>
      ...
      {/* Mobile Bottom Nav */}
      <nav className="md:hidden fixed bottom-0 left-0 right-0 bg-white border-t border-slate-100 flex justify-around items-center h-16 z-50 px-2 pb-safe">
         ...
         <Link to="/journey" className={`flex flex-col items-center space-y-1 ${isActive('/journey') ? 'text-indigo-600' : 'text-slate-400'}`}>
            ...
            <span className="text-[10px] font-bold uppercase">Journey</span>
         </Link>
         ...
         <a href="https://docs.google.com/spreadsheets/d/19RdOnf8wdOgk1o3HMovmaNrh8Qmhmg72PCPUlxkPa68/edit?usp=sharing" target="_blank" rel="noopener noreferrer" className="flex flex-col items-center space-y-1 text-slate-400">
            <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
